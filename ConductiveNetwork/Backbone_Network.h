...
public:
    ...
    //Member Functions
    int Determine_backbone_network(const int &family, const vector<GCH> &hybrid_particles, const vector<int> &cluster, const vector<double> &voltages, const vector<int> &LM_matrix, const vector<vector<long int> > &elements, const vector<vector<long int> > &structure, const vector<Point_3D> &points_in, vector<int> &cluster_gch, vector<double> &families_lengths, vector<double> &branches_lengths, vector<vector<long int> > &all_dead_indices, vector<vector<long int> > &all_indices, vector<vector<int> > &gnp_dead_indices, vector<vector<int> > &gnp_indices);
    double Zero_current(const vector<double> &voltages, const vector<int> &cluster, const vector<int> &LM_matrix, const vector<vector<long int> > &elements);
    int Find_dead_branches(const vector<double> &voltages, const vector<int> &cluster, const vector<int> &LM_matrix, const vector<vector<long int> > &elements, const double &zero_cutoff);
    ...
    int Find_dead_branches(const int &R_flag, Direct_Electrifying *DEA, const struct Electric_para &electric_param, const vector<int> &cluster, const vector<Point_3D> &point_list, const vector<double> &radii, const double &zero_cutoff);
    int Find_dead_branches_simplified(vector<vector<long int> > &elements, const vector<int> &cluster, const double &zero_cutoff);
    int Find_dead_gnps(const vector<double> &voltages, const vector<int> &LM_matrix, const vector<GCH> &hybrid_particles, const vector<vector<long int> > &structure, const double &zero_cutoff, vector<int> &cluster_gch, vector<int> &gnp_dead_indices, vector<int> &gnp_indices);
