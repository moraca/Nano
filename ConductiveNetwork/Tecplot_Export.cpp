...
//---------------------------------------------------------------------------
//The geometric structure of CNT network (by threads in Tecplot) in a cuboid
int Tecplot_Export::Export_network_threads(const struct cuboid &cub, const vector<Point_3D> &cnts_points, const vector<vector<long int> > &structure, string filename)const
{
...
//---------------------------------------------------------------------------
//The geometric structure of CNT network (by threads in Tecplot) in a cuboid
int Tecplot_Export::Export_triangulation_network_threads(const GCH &hybrid, const vector<Point_3D> &cnts_points, const vector<vector<long int> > &structure, string filename)const
{
    ofstream otec(filename.c_str());
    ...
    //---------------------------------------------------------------------------
    //Export 3D nanotube threads
    if(Export_triangulation_nano_threads(otec, hybrid.center, cnts_points, structure)==0) return 0;
...
//---------------------------------------------------------------------------
//Export 3D nanotube threads
int Tecplot_Export::Export_triangulation_nano_threads(ofstream &otec, const Point_3D &center, const vector<Point_3D> &cnts_points, const vector<vector<long int> > &structure)const
{
    //Number of points
    long int points = (long int)cnts_points.size();
    ...
    for(int i=0; i<(int)structure.size(); i++)
    {
        ...
        for (int j=0; j<(int)structure[i].size(); j++)
        {
            P = structure[i][j];
            if (P >= points) {
                //Use the center of the GNP
                otec << center.x << "  " << center.y << "  " << center.z << endl;
            } else {
                //Use a point from the list of points
...
//---------------------------------------------------------------------------
//The geometric structure of CNT network (by threads in Tecplot) in a cuboid
int Tecplot_Export::Export_hybrid_threads(const struct cuboid &cub, const vector<Point_3D> &cnts_points, const vector<GCH> &hybrid_particles, const vector<int> &cluster_gch, const vector<vector<long int> > &structure, string filename)const
{
    ofstream otec(filename.c_str());
    otec << "TITLE = CNT_Wires" << endl;
    otec << "VARIABLES = X, Y, Z" << endl;
    ...
    if (cluster_gch.size()) {
        string name = "GNPs";
        if(Export_randomly_oriented_gnps(otec, hybrid_particles, cluster_gch, name)==0) return 0;
    }
...
int Tecplot_Export::Export_cnts_meshes_singlezone(const struct cuboid &cub, const vector<vector<Node> > &nodes, const vector<vector<Element> > &eles)const
{
	...
	if(cnt_count!=(int)eles.size()) { hout << "节点和单元显示的纳米管线数量不一致！ 请检查！" << endl; return 0; }
...
int Tecplot_Export::Export_cnt_network_meshes(const struct cuboid &cub, const int &n_cluster, const vector<vector<int> > &gnp_clusters, const vector<vector<int> > &cnt_clusters, const vector<vector<long int> > &structure, const vector<Point_3D> &cnts_points, const vector<double> &cnts_radius, const vector<GCH> &hybrid_particles, string &filename, string &family)const
{
    ...
    if (!Export_hybrid_meshes_singlezone(cub, n_cluster, cnts_nodes, cnts_eles, hybrid_particles, gnp_clusters, filename, family)) {
...
int Tecplot_Export::Export_hybrid_meshes_singlezone(const struct cuboid &cub, const int &n_cluster, const vector<vector<Node> > &nodes, const vector<vector<Element> > &eles, const vector<GCH> &hybrid_particles, const vector<vector<int> > &gnp_clusters, string &filename, string &family)const
{
    //The total number of nanotube threads
    int cnts_account = (int)nodes.size();
    ...
    if (nodes.size()) {
        ...
        otec << "ZONE T=" << family << " N=" << nodes_num << ", E=" << eles_num << ", F=FEPOINT, ET=TETRAHEDRON" << endl;
        for(int i=0; i<cnts_account; i++)
        {
            for(int j=0; j<(int)nodes[i].size(); j++)
            {
