...
public:
    ...
    int Trim_boundary_cnts2(vector<vector<int> > &shells_cnt, int window, struct Geom_RVE sample, vector<Point_3D> &points_in, vector<vector<long int> > &structure, vector<double> &radii);
    int First_index(vector<Point_3D> &points_in, vector<long int> &structure_CNT, int &index1);
    int Second_index(vector<Point_3D> &points_in, vector<long int> &structure_CNT, int &index2);
