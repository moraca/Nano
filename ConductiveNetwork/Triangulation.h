...
public:
    ...
    int Single_surface_triangulation(const int &flag, const long int &GNP_center, const vector<Point_3D> &point_list, const vector<vector<long int> > &structure, GCH &hybrid);
    int Points_to_triangulate(const int &flag, const vector<vector<long int> > &structure, vector<long int> &points_out, GCH &hybrid);
    ...
    int Generate_supertriangle(const int &flag, const GCH &hybrid, vector<Point_3D> &vertices);
    int Initialice_triangulation(vector<vector<long int> > &triangles);
    int Bowyer_watson(const int &flag, const long int &GNP_center, const vector<Point_3D> &point_list, vector<Point_3D> &vertices, const vector<long int> &points_t, vector<vector<long int> > &triangles, GCH &hybrid);
    ...
    int Final_triangulation_edges(const int &flag, const long int &GNP_center, const vector<vector<long int> > &triangles, vector<vector<long int> > &edges);
    int Valid_edges(const int &flag, const long int &GNP_center, const vector<long int> &triangle, vector<vector<long int> > &edges);
    
    //3D triangulation
    int Generate_3d_trangulation(const vector<Point_3D> &point_list, const vector<vector<long int> > &structure, GCH &hybrid);
    int Generate_supertetrahedron(const GCH &hybrid, vector<Point_3D> &vertices);
